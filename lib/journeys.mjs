...
import {AREA_LABELS, safeTarget, TOWN_TARGETS} from '../public/journey-map.mjs';
...
export const TOWN_ACTIONS=['town_step','town_reset','town_switch','town_example','town_run','town_export'];
export {TOWN_TARGETS};
ACTIONS.push(...TOWN_ACTIONS, 'link_open', 'contents_open');
...
export function validJourneyBatch(batch, now = Date.now()) {
  ...
  return batch.events.every(e => {
    if (!e || Object.keys(e).sort().join(',') !== 'action,activeMs,area,at,seq,target' ||
        ...
        (TOWN_ACTIONS.includes(e.action)&&(e.area!=='studio'||(e.action==='town_step'?!TOWN_TARGETS.includes(e.target):e.target!=='studio-town'))) ||
        typeof e.target !== 'string' || safeTarget(e.target)!==e.target ||
