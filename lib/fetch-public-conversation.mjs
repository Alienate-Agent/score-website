...
export async function fetchBoardJson(path,fetcher=fetch){
  if(!/^\/api\/(?:post\/\d+|comment\/\d+|search\?q=[^&]+&limit=20|citizen\/(?:alienate|tidemark)(?:\?(?:posts_before|comments_before)=[1-9]\d*(?:&(?:posts_before|comments_before)=[1-9]\d*)?)?)$/.test(path))throw Error('unsupported');
