...
export async function fetchBoardJson(path,fetcher=fetch){
  if(!/^\/api\/(?:citizens(?:\?since=[1-9]\d*)?|grants|post\/\d+|comment\/\d+|search\?q=[^&]+&limit=20|citizen\/[a-z0-9][a-z0-9_-]{0,63}(?:\?(?:posts_before|comments_before)=[1-9]\d*(?:&(?:posts_before|comments_before)=[1-9]\d*)?)?)$/.test(path))throw Error('unsupported');
