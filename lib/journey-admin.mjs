...
export async function listSessions(db,o) {
  keys(o,['action','from','to','visitor','includeExcluded','after']);windowOf(o);
  ...
  const result=await db.prepare(`SELECT s.*, v.self_tester,
    ...
    .bind(o.from,o.to,visitor,visitor,Number(o.includeExcluded),after.at,after.at,after.id).all();
...
export async function handleJourneyAdmin(request,env) {
  ...
  let o;try{o=await bodyOf(request);keys(o,['action','from','to','visitor','includeExcluded','after','session','kind','value','reason','enabled','paused']);}catch{return reply(400,{error:'Invalid request'});}
  try {
    ...
    if(o.action==='overview') {
      ...
      return reply(200,{at:Date.now(),configured:ready(env),capacity:await capacityStatus(db),summary,actions:actions.results});
    }
