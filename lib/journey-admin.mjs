...
import {AREA_LABELS, MAP_VERSION} from '../public/journey-map.mjs';
...
export async function listSessions(db,o) {
  ...
  const area=o.area||'';
  if(area&&!Object.hasOwn(AREA_LABELS,area))throw Error('area');
...
export async function handleJourneyAdmin(request,env) {
  ...
  try {
    ...
    if(o.action==='overview') {
      ...
      const locations=await db.prepare(`SELECT e.area,e.target,COUNT(*) AS events,COUNT(DISTINCT e.session_id) AS sessions,
        SUM(CASE WHEN e.action IN ('view','section','navigate','return') THEN 1 ELSE 0 END) AS arrivals,
        SUM(e.active_ms) AS active_ms FROM journey_events e
        JOIN journey_session_classification s USING(session_id,visitor_id)
        WHERE e.received_at>=? AND e.received_at<? AND s.excluded=0
        GROUP BY e.area,e.target ORDER BY sessions DESC,e.area,e.target LIMIT 501`).bind(o.from,o.to).all();
      return reply(200,{at:Date.now(),configured:ready(env),capacity:await capacityStatus(db),summary,actions:actions.results,studio:studio.results,townInteractionReporting:true,
        journeyMapVersion:MAP_VERSION,availableAreas:Object.keys(AREA_LABELS),locations:locations.results.slice(0,500),locationsMore:locations.results.length>500});
    }
