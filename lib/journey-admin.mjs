...
export async function handleJourneyAdmin(request,env) {
  ...
  try {
    ...
    if(o.action==='overview') {
      ...
      const studio=await db.prepare(`SELECT e.target,COUNT(DISTINCT e.session_id) AS sessions,
        ...
        GROUP BY e.target ORDER BY sessions DESC,e.target LIMIT 20`).bind(o.from,o.to).all();
      return reply(200,{at:Date.now(),configured:ready(env),capacity:await capacityStatus(db),summary,actions:actions.results,studio:studio.results});
    }
