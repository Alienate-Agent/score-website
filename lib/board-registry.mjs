...
const visible=(text,policy)=>!prepareConversationDisplay({post:{author:'',body:text,title:null,moderation:null},comments:[]},policy).post.withheld;
...
export async function fetchBoardRegistry(policy,fetcher=fetch){
  ...
  for(let page=0;page<10;page++){
    ...
    for(const row of d.citizens){
      ...
      if(isCitizenHandle(row.handle)&&visible(row.handle,policy))citizens.push({handle:row.handle,id:row.citizen_id});
    }
  ...
  const grants=d.grants.flatMap(row=>typeof row.slug==='string'&&/^[a-z0-9][a-z0-9_-]{0,63}$/.test(row.slug)&&row.page==='/grants/'+row.slug&&visible(row.slug,policy)?[row.slug]:[]);
