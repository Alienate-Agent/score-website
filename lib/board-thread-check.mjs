...
export function compareThread(current,baseline){
  const comparable=!current.partial && !baseline.partial;
  return {thread_id:current.thread_id,source_time:current.source_time,comments_total:current.comments_total,comments_returned:current.comments_returned,partial:current.partial,baseline_time:baseline.source_time,text_comparable:comparable,text_changed:comparable?current.text_fingerprint!==baseline.text_fingerprint:null,count_changed:current.comments_total!==baseline.comments_total};
}
