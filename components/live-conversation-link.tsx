...
export function LiveConversationLink({postId,commentId,children}:{postId:number;commentId?:number;children:React.ReactNode}){
  const selected=commentId?`comment:${commentId}`:`post:${postId}`;
  ...
  async function show(){
    ...
    try{
      const response=await fetch(`/api/board-search?kind=${commentId?'comment':'post'}&id=${commentId??postId}`,{credentials:'omit',cache:'no-store',signal:controller.signal});
      if(!response.ok)throw Error('unavailable');
      const data:unknown=await response.json();
      if(!data||typeof data!=='object'||!('mode' in data)||data.mode!=='conversation'||!('selected' in data)||data.selected!==selected||!('conversation' in data)||!freshConversation(data.conversation,postId))throw Error('invalid');
