...
export function ReadingNavigation(){
 ...
 useEffect(()=>{
  try{setCollapsed(sessionStorage.getItem('score-story-collapsed')==='true');}catch{}
  let frame=0;
  ...
  return()=>{observer.disconnect();cancelAnimationFrame(frame);window.removeEventListener('hashchange',reveal);window.removeEventListener('scroll',schedule);window.removeEventListener('resize',schedule);};
 },[]);
 ...
 return <div className="reading-navigation" ref={bar}>
  <nav id="reading-trail" aria-label="Your reading trail" />
  {(section||collapsed)&&<div className="reading-section-bar" data-story-title={collapsed||section.startsWith('THE STORY')}>
