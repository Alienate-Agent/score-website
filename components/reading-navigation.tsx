...
export function ReadingNavigation(){
 ...
 useEffect(()=>{
  ...
  const update=()=>{frame=0;const title=document.querySelector('.story-so-far');const limit=(document.querySelector('.reading-help-bar')?.getBoundingClientRect().height||0)+40;
   ...
   const areas=[...document.querySelectorAll<HTMLElement>('[data-story-fold],#connected-score,#story-exploration,[data-reading-layer],#resources,#correspondence')].filter(e=>e.getClientRects().length&&e.getBoundingClientRect().top<=limit+120&&e.getBoundingClientRect().bottom>limit);
