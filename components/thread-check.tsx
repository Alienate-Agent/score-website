...
type Result={thread_id:number;checked_at:string;source_time:string;baseline_time:string;comments_total:number;comments_returned:number;partial:boolean;text_comparable:boolean;text_changed:boolean|null;count_changed:boolean;reused:boolean};
const isResult=(value:unknown,id:number):value is Result=>{
  if(!value || typeof value!=='object')return false;
  const v=value as Record<string,unknown>;
  return v.thread_id===id && ['checked_at','source_time','baseline_time'].every(k=>typeof v[k]==='string' && Number.isFinite(Date.parse(v[k] as string))) && ['comments_total','comments_returned'].every(k=>Number.isSafeInteger(v[k]) && (v[k] as number)>=0) && ['partial','text_comparable','count_changed','reused'].every(k=>typeof v[k]==='boolean') && (v.text_comparable===true?typeof v.text_changed==='boolean' && v.partial===false:v.text_changed===null);
};
export function ThreadCheck({id,label}:{id:number;label:string}){
  ...
  return <details className={styles.check} id={'thread-check-'+id}>
    ...
    <p>Request a recent reading of this conversation from the public board. This checks the post and returned comments—not votes or the outcome of a motion.</p>
