...
for(const path of ['/agent-guide','/agent-words','/board','/changelog','/featured','/charter','/charter/','/future-page']){
  pathname=path;
...
assert.match(read('../app/layout.tsx'),/<SiteMasthead\s*\/>\{children\}/);
assert.match(read('../app/site-masthead.css'),/position:sticky;top:0/);
assert.match(read('../app/site-masthead.css'),/scroll-margin-top:var\(--site-masthead-offset\)/);
...
console.log('PASS: SCORE word reveal at entrance, TAASO after claim scrolls away, shared secondary/charter masthead, all Studio host pages and instrument covered.');
