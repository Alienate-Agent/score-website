...
assert.ok(quotation.includes('data-omission="selected"'),'Known quotations are distinguished from reserved space');
assert.ok(!quotation.includes('length:'),'No imitation of hidden line counts');
...
assert.equal((story.match(/<WithheldAccount \/>/g)||[]).length,2,'Both future accounts remain reserved, not silently removed');
const withholding=read('components/withheld-account.tsx');
assert.ok(withholding.includes('data-omission="reserved"'));
assert.ok(withholding.includes('No passage selected'));
assert.ok(withholding.includes('export function WithheldAccount()'),'Reserved account accepts no private material');
const prelude=read('components/prelude-conversation.tsx');
