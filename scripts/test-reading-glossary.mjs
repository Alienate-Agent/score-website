...
assert.ok(help.includes("from '@/components/reading-help-context'"));
assert.ok(!help.includes('createContext'));
assert.ok(help.includes('observer.disconnect()'));
assert.ok(help.includes("root.style.removeProperty('--reading-help-height')"));
assert.ok(read('components/reading-help-context.ts').includes('createContext'));
const css=read('components/reading-glossary.css');
assert.ok(css.includes('scroll-padding-top:var(--reading-help-height,0px)'));
assert.ok(css.includes('.reading-help-bar .reading-glossary-launch { position:static;'));
