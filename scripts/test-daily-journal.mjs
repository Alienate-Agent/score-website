...
assert.equal(dailyJournal.length,17);
...
for(const p of dailyJournal){
 ...
 for(const text of ['How this was drawn','Original words &amp; sources','Edition history','2026-09-25T21:33:05.448Z','/record#score-privacy','og:image','og:description','Copy link'])assert(html.includes(text),p.date+' '+text);
...
assert(fs.readFileSync('public/entrance/index.html','utf8').includes('/journal/2026-09-25'));
console.log('PASS 17 daily routes, all source references, 11 frozen drawing hashes, family recurrence, metadata and legacy routing.');
