...
assert.equal(dailyJournal.length,14);
...
for(const p of dailyJournal){
 ...
 for(const text of ['How this was drawn','Original words &amp; sources','Edition history','2026-09-22T21:35:11Z','/record#score-privacy','og:image','og:description','Copy link'])assert(html.includes(text),p.date+' '+text);
...
assert(fs.readFileSync('public/entrance/index.html','utf8').includes('/journal/2026-09-22'));
console.log('PASS 14 daily routes, all source references, 11 frozen drawing hashes, family recurrence, metadata and legacy routing.');
